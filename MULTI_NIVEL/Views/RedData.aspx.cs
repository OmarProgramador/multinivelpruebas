...
namespace MULTI_NIVEL.Views
{
    ...
    public partial class RedData : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < arrayD.Length; i++)
            {
                ...
                if (padre1 == padre)
                {
                    partners1.Add(new Partner1 { id = int.Parse(row[1]), name = row[2], title = $"<span class='{classCss}'> • </span> " + ConvertStatus(row[4]) + " <br>" + row[3] });
                }
            }
