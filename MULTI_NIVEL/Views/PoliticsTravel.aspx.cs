...
{
    public partial class PoliticsTravel : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    var option = Session["option"].ToString();
                    if (!string.IsNullOrEmpty(option))
                    {
                        if (option == "ug")
                        {
                            BrTypeMembership brTypeMembership = new BrTypeMembership();
                            correlativo = int.Parse(brTypeMembership.GetTotalMemberships(User.Identity.Name.Split('¬')[1])).ToString();
        ...
        {
            if (cbagree.Checked && cbagree2.Checked && cbagree3.Checked /*&& cbagree4.Checked*/)
            {
